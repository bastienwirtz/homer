...
const GROUPS = ["structure", "vocabulary", "mixin", "errors", "style"];

// `card-contract: off [groups] -- why`; naming no group turns every check off.
const MARKER = /card-contract:\s*off([^\n\r]*)/;
...
function readMarker(context) {
  const match = MARKER.exec(context.sourceCode.getText());
  if (!match) {
    return { off: new Set(), problems: [] };
  }

  const loc = context.sourceCode.getLocFromIndex(match.index);
  const [named, reason] = match[1].replace(/(-->|\*\/)\s*$/, "").split("--");
  const problems = [];
  ...
  if (!reason?.trim()) {
    problems.push({
      loc,
      message:
        'An exemption must say why: "card-contract: off <groups> -- <reason>". Without a reason nobody can tell later whether it still applies.',
    });
  }

  const wanted = named.split(/[\s,]+/).filter(Boolean);
  const unknown = wanted.filter((group) => !GROUPS.includes(group));
  if (unknown.length) {
    problems.push({
      loc,
      message: `Unknown check group "${unknown.join('", "')}". Available: ${GROUPS.join(", ")}.`,
    });
  }

  // A malformed marker exempts nothing, so a typo cannot silence the contract.
  if (problems.length) {
    return { off: new Set(), problems };
  }

  return { off: new Set(wanted.length ? wanted : GROUPS), problems };
}
...
export default {
  ...
  create(context) {
    ...
    const { off, problems } = readMarker(context);

    return {
      Property(node) {
        if (off.has("vocabulary")) {
          return;
        }
      ...
      ExportDefaultDeclaration(node) {
        ...
        if (!off.has("mixin")) {
          ...
          for (const option of ["data", "computed", "methods"]) {
            const keys =
              option === "data" ? data : optionKeys(component, option);
            for (const shadowed of keys.filter((k) =>
              MIXIN_NAMES.includes(k),
            )) {
              context.report({
      ...
      Program() {
        ...
        for (const block of fragment.children) {
          ...
          if (block.name === "template" && !off.has("structure")) {
          ...
          if (block.name === "style" && !off.has("style")) {
